...
function getAnalyzer() {
    ...
    function addLexeme(lexeme, line, type, constIndex) {
        ...
        if (lexemesList.includes(lexeme)) {
            ...
            if (!declaredID) {
                return index = addVar(lexeme, line);
            } else {
    ...
    function addVar(lexeme, line) {
        ...
        addLexeme(lexeme, line, lexType.ID, index)
        currentLexeme = '';
        return index;
    }
    ...
    function isCharacter(char) {
        return !isDelimiter(char) && /[a-zA-Z_$]/.test(char);// char >= 'A' && char <= 'z';
    }
...
// initial state
    function mode1(char) {
        ...
        if (isDelimiter(char)) {
            ...
            if (char === '\n') {
                addLexeme(char, currentLine, lexType.RESERVED);
                currentLine++;
...
// fixed to dot
    function mode3(char) {
        ...
        if (isCharacter(char)) {
            throw new Error(`Variable cannot start with a number! line:${currentLine+1}`);
        }
...
// -
    ...
    function analyze(code) {
        ...
        // fs.writeFileSync('../public/tables.json', JSON.stringify({
        //     lexemesTable
        // }));
        return {
